...
public class MarkServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        double[] passFailRatio = markDAO.getPassFailRatio();
        request.setAttribute("passRatio", passFailRatio[0]);
        request.setAttribute("failRatio", passFailRatio[1]);

        RequestDispatcher rd = request.getRequestDispatcher("/views/admin/score.jsp");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (add > 0) {
            response.sendRedirect(request.getContextPath() + "/admin/mark?msg=OK");
            return;
        } else {
            response.sendRedirect(request.getContextPath() + "/admin/mark?msg=ERROR");
            return;
        }
    }
